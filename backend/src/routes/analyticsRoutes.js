...
const { protect, wholesalerOnly, shopkeeperOnly } = require('../middleware/authMiddleware');

// Shared aggregation used by both the wholesaler view (their sales) and the
// shopkeeper view (their spending) — same shape either way, just scoped by
// a different `where` clause.
async function buildAnalytics(where) {
  ...
  // Aggregate quantity/amount per product across every order that wasn't
  // rejected (Rejected orders never actually shipped).
  const productMap = {};
  for (const order of orders) {
  ...
  // Delivered amount for each of the last 14 days
  const DAYS = 14;
  const salesByDay = [];
...
router.get('/wholesaler', protect, wholesalerOnly, async (req, res) => {
  try {
    res.json(await buildAnalytics({ wholesalerId: req.user.id }));
  } catch (error) {
    console.error(error);
...
// @desc    Spending analytics for the logged-in shopkeeper: spend summary,
//          order status breakdown, most-ordered products, and a 14-day
//          spend trend.
// @access  Private - Shopkeeper only
router.get('/shopkeeper', protect, shopkeeperOnly, async (req, res) => {
  try {
    res.json(await buildAnalytics({ shopkeeperId: req.user.id }));
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Error fetching analytics' });
  }
